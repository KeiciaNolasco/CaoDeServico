...
@Component({
  selector: 'app-documentacaocustomer',
...
export class DocumentacaoCustomerComponent implements OnInit {
  id!: number;
  ...
  update(): void {
    if (this.documentacao) {
      this.router.navigate(['/documentacaoupdate', this.id]);
    } else {
      this.errorMessage = 'Documentação não encontrada para editar.';
      this.router.navigate(['/documentacaosave', this.id]);
    }
  }

...
export class EnderecoAdminComponent implements OnInit {
  id!: number;
  ...
  update(): void {
    if (this.endereco) {
      this.router.navigate(['/enderecoupdateadmin', this.id]);
    } else {
      this.errorMessage = 'Endereço não encontrado para editar.';
      this.router.navigate(['/enderecosaveadmin', this.id]);
    }
  }

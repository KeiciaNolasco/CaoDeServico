...
export class CaoAdminComponent implements OnInit {
  id!: number;
  cao: Cao| undefined; 
  ...
  update(): void {
    if (this.cao) {
      this.router.navigate(['/caoupdateadmin', this.id]);
    } else {
      this.errorMessage = 'Cão de Serviço não encontrado para editar.';
      this.router.navigate(['/caosaveadmin', this.id]);
    }
  }
  ...
  onConfirmDelete(confirm: boolean): void {
    ...
    if (confirm) {
      this.caoService.delete(this.id).subscribe({
        next: () => {
          this.router.navigate(['/caoadmin', this.id]);
        },
        error: (err) => {
          this.router.navigate(['/caoadmin', this.id]);
        }
